 ...
 * <li><code>opencms.title</code> returns the title of the document that should be used for the 
 ...
 * <li><code>opencms.description</code> returns the description of the document that should be used for the 
 * HTML description meta tag. This is useful for container detail pages, in which case it will return the Description 
 * of the detail, not the container page. Otherwise it just returns the value of the Description property.
 * <li><code>opencms.keywords</code> returns the keywords of the document that should be used for the 
 * HTML keywords meta tag. This is useful for container detail pages, in which case it will return the Keywords 
 * of the detail, not the container page. Otherwise it just returns the value of the Keywords property.
 * </ul>
 * 
...
public class CmsJspTagInfo extends TagSupport {
    ...
    /**
     * Returns the title of a page delivered from OpenCms, usually used for the meta tag of
     * a HTML page.<p>
     * 
     * If no title information has been found, the empty String "" is returned.<p>
     * 
    ...
    public static String getTitleInfo(CmsFlexController controller, HttpServletRequest req) {
        ...
        try {
            ...
            if (contextBean.isDetailRequest()) {
                ...
                CmsXmlContent content = CmsXmlContentFactory.unmarshal(cms, res, req);
                //result = content.getHandler().getTitleMapping(cms, content, cms.getRequestContext().getLocale());
                LOG.debug("get field MetaTitle content...");
                if(content.getValue("MetaTitle", cms.getRequestContext().getLocale(), 0) != null){
                	result = content.getValue("MetaTitle", cms.getRequestContext().getLocale(), 0).getStringValue(cms);
                	LOG.debug("get field MetaTitle content : " + result);
                }
                if (result == null) {
                    // title not found, maybe no mapping OR not available in the current locale
                    // read the title of the detail resource as fall back (may contain mapping from another locale)
            ...
            if (result == null) {
                // read the description of the requested resource as fall back
                result = cms.readPropertyObject(
                    cms.getRequestContext().getUri(),
                    CmsPropertyDefinition.PROPERTY_TITLE,
                    true).getValue();
    ...
    /**
     * Returns the description of a page delivered from OpenCms, usually used for the meta tag of
     * a HTML page.<p>
     * 
    ...
    public static String getDescriptionInfo(CmsFlexController controller, HttpServletRequest req) {
        ...
        try {
            ...
            if (contextBean.isDetailRequest()) {
                ...
                CmsXmlContent content = CmsXmlContentFactory.unmarshal(cms, res, req);
                //result = content.getHandler().getTitleMapping(cms, content, cms.getRequestContext().getLocale());
                LOG.debug("get field MetaDescription content...");
                if(content.getValue("MetaDescription", cms.getRequestContext().getLocale(), 0) != null){
                	result = content.getValue("MetaDescription", cms.getRequestContext().getLocale(), 0).getStringValue(cms);
                	LOG.debug("get field MetaDescription content : " + result);
                }
                if (result == null) {
                    // description not found, maybe no mapping OR not available in the current locale
    ...
    /**
     * Returns the keywords of a page delivered from OpenCms, usually used for the meta tag of
     * a HTML page.<p>
     * 
     * If no keywords information has been found, the empty String "" is returned.<p>
     * 
     * @param controller the current OpenCms request controller
     * @param req the current request
     * 
     * @return the keywords of a page delivered from OpenCms
     */
    public static String getKeywordsInfo(CmsFlexController controller, HttpServletRequest req) {
        ...
        try {
            ...
            if (contextBean.isDetailRequest()) {
                ...
                CmsXmlContent content = CmsXmlContentFactory.unmarshal(cms, res, req);
                //result = content.getHandler().getTitleMapping(cms, content, cms.getRequestContext().getLocale());
                LOG.debug("get field MetaKeywords content...");
                if(content.getValue("MetaKeywords", cms.getRequestContext().getLocale(), 0) != null){
                	result = content.getValue("MetaKeywords", cms.getRequestContext().getLocale(), 0).getStringValue(cms);
                	LOG.debug("get field MetaKeywords content : " + result);
                }
                if (result == null) {
                    // keywords not found, maybe no mapping OR not available in the current locale
            ...
            if (result == null) {
                // read the keywords of the requested resource as fall back
                result = cms.readPropertyObject(
                    cms.getRequestContext().getUri(),
